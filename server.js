...
app.put("/authors/:id", (req, res) => {
  ...
  Authors.findOne({
    ...
    if (author) {
      ...
      Authors.findByIdAndUpdate(req.params.id, { $set: updated }, { new: true })
        .then(updatedAuthor => {
          res.status(200).json({
            id: updatedAuthor.id,
...
app.delete("/authors/:id", (req, res) => {
  BlogPosts.remove({ author: req.params.id })
    .then(() => {
      Authors.findByIdAndRemove(req.params.id).then(() => {
        console.log(
          `Dleted blog posts owned by and author with id ${req.params.id}`
        );
...
app.delete("/posts/:id", (req, res) => {
  BlogPosts.findByIdAndRemove(req.params.id).then(() => {
